...
resource "azurerm_automation_schedule" "example" {
  ...
  name                    = "${var.schedulename}-${count.index}"
  resource_group_name     = azurerm_resource_group.example.name
  automation_account_name = azurerm_automation_account.example.name
  frequency               = "Hour"
  timezone                = "Eastern Standard Time"
  #start_time              = local.schedule_start_times[count.index]
  start_time              = timeadd("${timestamp()}", "${format("%d", (var.schedule_interval_minutes * count.index) + var.schedule_interval_minutes)}m")
...
resource "azurerm_automation_job_schedule" "example" {
  ...
  resource_group_name     = azurerm_resource_group.example.name
  automation_account_name = azurerm_automation_account.example.name
  schedule_name           = "${var.schedulename}-${count.index}"
